...
> **第六次 M5 `20260824T055032.238341323Z`:**body 方向候选首次达到
> 1 个真实航点(`path=1.0199m`),但未达 3 航点。MCAP 显示运动窗内
> `slam_frozen` 在 77.5/85.0/91.5s 三次误拦稳定低速推进;根因是 1.5s/2cm 窗口
> 比真实约 1cm/s 速度更苛刻。controller 又对每条 intent 使用 `dt<=0.25s`,
> adapter 2Hz 使目标推进速度折半。第六次 bag 离线反例还证明 4s 净位移窗仍会
> 误杀弧线回转;替换方法因此是 adapter 4Hz + 从当前航点/连续命令 epoch 起算的
> 8s/2cm 停滞闩锁。同一 bag 离线复验 62 个完整新窗口,误触发 0,最小真实推进
> 0.047439m;门槛与运行证据之间有明确间隔。不放宽 3 航点、返航、降落或停桨门。
> **`20260824T074701.195741657Z` 继续订正:**第 1 个航点后切换至第 2 个航点时,
> 飞行器制动回撤后再推进,窗口内相对起点最大位移约 0.08m,但 8.2s 首尾
> 净位移仅 0.012m,旧判据因此误闩锁 `slam_frozen`。替换方法是对当前 epoch
> 计算“任一样本相对起点的最大位移”,仍以 8s/0.02m 作为真正冻结的
> fail-closed 门槛。新 M5 `20260824T075515.387084602Z` 已在同一窗口证明
> 3 个真实航点、4.0871m 路径和完整返航/LAND/停桨链,`TASK_STATUS_OK`,
> acceptance rc=0。该结论只关闭 P1-2 直连闭环,不替代 M5-c 多 run 对比。

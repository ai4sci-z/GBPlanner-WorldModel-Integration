...
def launch_spawn_robot(context: LaunchContext) -> List[LaunchDescriptionEntity]:
    ...
    spawn_robot = Node(
        ...
        arguments=[
            ...
            "-name",
            name,
            # NOTE(navlab-humble-fix): spawn from file, not the RSP topic.
            # humble's sdformat_urdf cannot parse the gpu_lidar <sensor> and RSP
            # is fed a sensor-stripped description (see below), so the topic no
            # longer carries the full model; the file does.
            "-file",
...
def launch_state_pub_with_bridge(
    ...
    # Load SDF file.
    # NOTE(navlab-humble-fix): flatten <include>s first (gz sdf -p) so that the
    # sensor-strip below also removes sensors living in included sub-models
    # (e.g. lidar_2d's gpu_lidar). Fall back to the raw file if gz is missing.
    ...
    # NOTE(navlab-humble-fix): gz sdf -p (Harmonic/sdformat14) emits <sdf version="1.11">,
    # but humble's libsdformat in sdformat_urdf only accepts <=1.9 and RSP fails with
    # "Unable to convert from SDF version 1.11 to 1.9". The flattened ardupilot models
    # use no 1.10+ features, so relabeling the version is safe here.
    import re as _re2
    robot_desc = _re2.sub(r'<sdf version="[^"]+"', '<sdf version="1.9"', robot_desc, count=1)

    robot_desc = robot_desc.replace(
        "<fdm_port_in>9002</fdm_port_in>", f"<fdm_port_in>{control_port}</fdm_port_in>"
    )

    # NOTE(navlab-humble-fix): strip <sensor> blocks for robot_state_publisher only.
    # humble's sdformat_urdf raises "sensor type missing or invalid" on gpu_lidar and
    # RSP terminates, which kills /robot_description, TF and (previously) the spawn.
    # TF only needs links/joints; the full SDF still reaches Gazebo via spawn -file.
    import re as _re
    robot_desc = _re.sub(r"<sensor\b[^>]*/>", "", robot_desc)
    robot_desc = _re.sub(r"<sensor\b.*?</sensor>", "", robot_desc, flags=_re.DOTALL)

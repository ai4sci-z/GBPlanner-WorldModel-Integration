...
> 状态:REFERENCE / ACTIVE-for-Migration(2026-07-08)
> 读者:要把 GBPlanner 从 ROS1 移植成 ROS2 原生节点、并直接接进 world-model 的人。
> 定位:这是 1/2/3 三份精读报告的**入口与索引**,不重复细节。细节各归各篇。
> 关联:主线任务书 [GBPlanner_ROS2原生迁移可行性与任务拆解_2026-07-08.md](GBPlanner_ROS2原生迁移可行性与任务拆解_2026-07-08.md) · 路线切换 [路线切换_ROS2迁移_2026-07-07.md](路线切换_ROS2迁移_2026-07-07.md)。
...
4. **GBPlanner 的落点 = 替换"决策框"(intent 的生产者)**,其余链路原样复用。插拔靠 `strategy` 字段:原版只是标签,真正的让位机制是 clean 分支加的 **`strategy=external` 分支**——内建 workflow 整段让位,把 intent 总线与 status 话题所有权交给外部进程。详见 [理解(三)](worldmodel理解_3_探索策略接口_GBPlanner接入点.md)。
...
- **行号基准差异**:理解(一)以 WSL `~/ws-clean/world-model` @ `e7ca9fc` 为准(含 B16 探针修复+预算调整);理解(二)/(三) 部分以 Windows `sources/world-model-源码/` 上游快照为准(**原版无 external 分支**)。凡涉及 clean 分支修复处各篇已显式标注。

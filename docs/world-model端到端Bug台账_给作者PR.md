...
|---|---|---|---|---|---|
| OPEN-1 | (未定案,勿编 B 号) | 间歇性 bring-up 失败(wm `eab0cc6`,默认主线 6/3/3,诊断臂旁证 4/3/3,两 profile 分层统计)。两类失败:BIN-present(prearm 循环未过;`Arm: Accels inconsistent` 文本在成功与失败 run 计数均=20,**非判别器**)/ no-BIN(SITL 无 BIN,死因 UNKNOWN;SITL 控制台未落盘是主观测缺口)。**新数据点(2026-07-21,AA001 受控 A/A,wm 9a1ce95)**:OFF 臂 2 攻 1 过——失败 run 20260721T133519(墙钟 3m48s,TASK_STATUS_BLOCKED)为 **no-BIN 类首次受控复现**,且带具体 blocker `hover_mission_abort:waiting_for_fcu_external_nav`+airborne 缺失+高度crosscheck 失败;成功对照 20260721T133202 同配置有 BIN。tlog 在盘可供 E0 提取器分析;**定向分析已完成(2026-07-21)**:最强候选链=FCU LOCAL_POSITION_NED 慢启动爬升期>mission 等待预算(60s/连续5s)→ready 抖动→abort→未 arm→(LOG_DISARMED=0)no-BIN——候选统一解释 no-BIN/间歇性/同 commit 并存,候选非 CONFIRMED;判别=tlog 帧级前 40s 速率(0.8-2.6 vs 1.75-5Hz);报告=runbooks/world-model-jazzy/l0_hover/open1/AA001_r3_定向分析_2026-07-21.md;可证伪预测见报告 §4;**AA003 前置已落地(2026-07-22,wm@6d412a11:LOG_DISARMED=1 进 profile+生成链验证+观测门)——下次失败臂将有 BIN/EKF 内部时序;仍 UNKNOWN:FCU 慢启动上游原因**;**机制细化(§6,time_boot_ms 直测)**:RTF 两 run 恒 0.30(排除本对样本的负载抖动),判别=FCU 流爬升速度×时钟域错配(墙钟 1000ms 阈值 ⇔ RTF0.3 下需 ≥3.33Hz 仿真钟速率;FAIL 首窗 3.0Hz 恰骑线);历史 RTF0.08 时代有效阈值 12.5Hz(sim)→no-BIN 高发自洽;**★AA003 结果(2026-07-22,真实 execute)**:aa-r1 OFF 失败(preflight_timeout/never arm),LOG_DISARMED=1 交付首个失败样本+BIN(3.15MB 含 XKF/NKF/VISO)。BIN+rosbag 分析(AA003_BIN_EKF分析_2026-07-22.md)**推进候选到直接证据级**:FCU/EKF 侧早期健康(输入 66.9Hz/EKF 3.3s 收敛/origin 早设)→**CONTRADICTED 输入慢/EKF融合慢/origin慢**;rosbag 证 RTF=0.30、ROS 侧 external nav 全程新鲜(**"external-nav 40s 硬停"是 sim/wall 时钟误读=run 末,SUPERSEDED**);**直接观测 sender `ready` 每1-2s翻转而 odom 恒新鲜1ms→翻转项 local_position_fresh(推断)→FCU LP 输出 0.9Hz(511 限速)→readiness 零迟滞门攒不够5s→timeout**。仍 UNKNOWN(翻转项属推断/511 为何慢);A/A 无结论(ON 未跑);**OPEN-1 未定位** | 竞争假设 H1-H5 与实验设计见 [WP304 方案](../governance/WP304_OPEN-1因果时间线与实验设计_2026-07-17.md) | 未修——按 WP304 分阶段(E1 观测→pilot→E2 对照),10/10 验收 | 🔴 OPEN,封 GATE-4b 稳定性宣称 |

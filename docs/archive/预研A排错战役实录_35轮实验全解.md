> **[HISTORICAL]** 35 轮实验实录(方法论价值)。当前状态以 [CURRENT_STATUS.md](../../CURRENT_STATUS.md) 为准。
...
## 五、当前战线与下一步
- ✅ **坑#14 根因锁定并修复(读源码)**:`fcu_controller_runtime.py.tmpl:192` 用
  `mode_id = master.mode_mapping().get("GUIDED") or guided_mode`——pymavlink 的 `mode_mapping()`
  依车型识别,SITL 早期握手可能返回 **ArduPlane 表(GUIDED=15=Copter 的 AUTOTUNE)**→ set_mode(15) 被拒。
  修复:改为**优先信显式配置** `guided_mode`(config=4,ArduCopter GUIDED),仅在其无效时回退 mode_mapping
  (world-model 分支 commit `b13f268`,go build 过)。run#36 端到端验证中。
- ◉ 次要:`PreArm: VisOdom not healthy`——若解锁仍卡,查 EKF3_SRCx/VISO 参数与 external_nav 健康门限。
- ○ 解锁起飞 → frontier_lite 真实指标 → 预研A 终点 → 接 `gbplanner_gain`/桥接对比。

> 方法论收束:坑#14 是"**先读源码、后动手**"的范例——没跑一轮盲目实验,直接读生成脚本定位到那一行 `mode_mapping()`,
> 对照 ArduPlane/ArduCopter 模式表(Plane GUIDED=15 / Copter GUIDED=4)即锁定根因。这正是要求13("多因没读源码陷死循环")的正解。

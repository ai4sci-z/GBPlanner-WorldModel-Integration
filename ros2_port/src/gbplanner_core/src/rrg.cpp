...
#include "planner_common/port/tf_convert.h"

#define SQ(x) (x * x)

namespace explorer {

Rrg::Rrg(MapManagerVoxblox<MapManagerVoxbloxServer, MapManagerVoxbloxVoxel>*
             map_manager)
...
void Rrg::initializeAttributes() {
  ...
  rostime_start_ = pshim::Time::now();
  //
  add_frontiers_to_global_graph_ = false;
  //
  exploring_direction_ = 0.0;
  // M3: the four ROS1 self-owned timers are gone; the ROS2 shell drives
  // tickPlanner()/tickFreePointCloud()/tickGlobalGraphUpdate()/
  // tickGlobalGraphFrontierAddition() at the periods declared in rrg.h.
  ...
  // M3: semantic_location / stop_request subscriptions moved to the shell,
  // which forwards into semanticsCallback()/stopMsgCallback().
}
...
void Rrg::freePointCloudtimerCallback(const pshim::TimerEvent& event) {
  ...
  for (auto sensor_name : free_frustum_params_.sensor_list) {
    ...
    // builtin_interfaces stamp from the shim wall clock
    const double now_s = pshim::Time::now().toSec();
    out_cloud.header.stamp.sec = static_cast<int32_t>(now_s);
    out_cloud.header.stamp.nanosec =
        static_cast<uint32_t>((now_s - static_cast<int32_t>(now_s)) * 1e9);
    io_.publishFreeCloud(out_cloud);
  }
}
...
void Rrg::expandGlobalGraphTimerCallback(const pshim::TimerEvent& event) {
  // Algorithm:
...
bool Rrg::loadParams(bool shared_params) {
  ...
  // parameters.
  // ROS1 prefixed params with the node name; the pshim param registry is
  // flat, so the shell stores keys under this fixed prefix.
  std::string ns = "/gbplanner_node";
...
bool Rrg::setGlobalBound(
  ...
  if (bound.reset_to_default) {
    ...
    if (!io_.lookupTransform(world_frame_, bound.header.frame_id,
                             &darpa_to_world_transform)) {
      ROS_ERROR_COND(global_verbosity >= Verbosity::ERROR,
                     "tf lookup %s -> %s unavailable", world_frame_.c_str(),
                     bound.header.frame_id.c_str());
    }
    // Center
...
std::vector<geometry_msgs::msg::Pose> Rrg::getBestPath(std::string tgt_frame,
  ...
  if (planning_params_.auto_landing_enable &&
    ...
    if ((pshim::Time::now()).toSec() != 0.0) {
      if (rostime_start_.toSec() == 0.0) rostime_start_ = pshim::Time::now();
      time_elapsed = (double)((pshim::Time::now() - rostime_start_).toSec());
    }
    ...
    if (time_budget_remaining <= 0.0) {
      ...
      if (!landing_engaged_) {
        io_.callLanding();
        io_.publishPciReset(stop_msg);
      }
      /* TODO: What happens if the disarm service fails */
      status = planner_msgs::srv::PlannerSrv::Response::K_FORWARD;
      return empty_path;
    }
  }
  // Decide if need to go home.
  if (planning_params_.auto_homing_enable) {
    status = planner_msgs::srv::PlannerSrv::Response::K_HOMING;
...
void Rrg::timerCallback(const pshim::TimerEvent& event) {
  // Re-initialize until get non-zero value.
  if (rostime_start_.toSec() == 0) rostime_start_ = pshim::Time::now();
...
std::vector<geometry_msgs::msg::Pose> Rrg::reRunGlobalPlanner() {
...
std::vector<geometry_msgs::msg::Pose> Rrg::runGlobalPlanner(int vertex_id,
  ...
  if (vertex_id) {
    pshim::Duration(3.0).sleep();  // sleep to unblock the thread to get and
...
void Rrg::addGeofenceAreas(const geometry_msgs::msg::PolygonStamped& polygon_msgs) {
  if ((planning_params_.geofence_checking_enable) && (planner_trigger_count_)) {
    ...
    if (!polygon_msgs.header.frame_id.compare(
      ...
      // Look up the tf (shell-injected; shell owns waiting policy).
      tf2::Transform tf_to_global;
      if (io_.lookupTransform(planning_params_.global_frame_id,
                              polygon_msgs.header.frame_id, &tf_to_global)) {
...
std::vector<geometry_msgs::msg::Pose> Rrg::searchPathToPassGate() {
  ...
  if (darpa_gate_params_.load_from_darpa_frame) {
    // Look up the tf (shell-injected).
    tf2::Transform tfW2G;
    if (io_.lookupTransform(darpa_gate_params_.world_frame_id,
                            darpa_gate_params_.gate_center_frame_id, &tfW2G)) {

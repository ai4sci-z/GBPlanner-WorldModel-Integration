...
bool RandomSamplerBase::loadParams(std::string ns) {
  ...
  if (sample_mode_ == SampleModeType::kManual) {
    if (pdf_type_ == RandomDistributionType::kUniform) {
      ...
      if (!pshim::param::get(param_name, min_val_)) {
        ROSPARAM_ERROR(param_name);
        return false;
      }
      param_name = ns + "/max_val";
      if (!pshim::param::get(param_name, max_val_)) {
        ROSPARAM_ERROR(param_name);
        return false;
      }
      ...
      if (!pshim::param::get(param_name, mean_val_)) {
        ROSPARAM_ERROR(param_name);
        return false;
      }
      param_name = ns + "/std_val";
      if (!pshim::param::get(param_name, std_val_)) {
        ROSPARAM_ERROR(param_name);
        return false;
      }
      ...
      if (!pshim::param::get(param_name, const_val_)) {
        ROSPARAM_ERROR(param_name);
        return false;
      }
    }

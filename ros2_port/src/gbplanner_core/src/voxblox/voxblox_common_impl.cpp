...
MapManagerVoxblox<SDFServerType, SDFVoxelType>::MapManagerVoxblox(
  ...
  if (!node_ptr->get_parameter("occupancy_distance_voxelsize_factor",
  ...
  tsdf_integrator_config_ =
      voxblox::getTsdfIntegratorConfigFromRosParam(node_ptr);
  esdf_integrator_config_ =
      voxblox::getEsdfIntegratorConfigFromRosParam(node_ptr);

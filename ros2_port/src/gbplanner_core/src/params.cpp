...
bool SensorParamsBase::loadParams(std::string ns) {
  ...
  param_name = ns + "/type";
  pshim::param::get(param_name, parse_str);
  ...
  if (!pshim::param::get(param_name, min_range)) {
    min_range = 0.0;
    ROSPARAM_WARN(param_name, min_range);
  }

  param_name = ns + "/max_range";
  if (!pshim::param::get(param_name, max_range)) {
    max_range = 5.0;
  ...
  if ((!pshim::param::get(param_name, param_val)) || (param_val.size() != 3)) {
    param_val.resize(3);
    param_val[0] = 0.0;
    param_val[1] = 0.0;
    param_val[2] = 0.0;
  ...
  param_val.clear();
  param_name = ns + "/fov";
  if ((!pshim::param::get(param_name, param_val)) || (param_val.size() != 2)) {
    param_val.resize(2);
    param_val[0] = 1.57;     // 90
  ...
  if (!pshim::param::get(param_name, frontier_percentage_threshold)) {
    frontier_percentage_threshold = 0.1;
    ROSPARAM_WARN(param_name, frontier_percentage_threshold);
  }

  param_name = ns + "/frame_id";
  if (!pshim::param::get(param_name, frame_id)) {
    frame_id = "";
    ROSPARAM_WARN(param_name, frame_id);
  }

  param_name = ns + "/width";
  if (!pshim::param::get(param_name, width)) {
    width = 0;
    ROSPARAM_WARN(param_name, frame_id);
  }

  param_name = ns + "/height";
  if (!pshim::param::get(param_name, height)) {
    height = 0;
    ROSPARAM_WARN(param_name, frame_id);
  }

  param_name = ns + "/width_removal";
  if (!pshim::param::get(param_name, widthRemoval)) {
    widthRemoval = 0;
    ROSPARAM_WARN(param_name, frame_id);
  }

  param_name = ns + "/height_removal";
  if (!pshim::param::get(param_name, heightRemoval)) {
    heightRemoval = 0;
    ROSPARAM_WARN(param_name, frame_id);
  }
...
bool SensorParams::loadParams(std::string ns) {
  ...
  pshim::param::get(param_name, parse_str_list);
...
bool RobotParams::loadParams(std::string ns) {
  ...
  if (!pshim::param::get(param_name, relax_ratio)) {
    relax_ratio = 0.5;
...
bool BoundedSpaceParams::loadParams(std::string ns) {
  ...
  if (type == BoundedSpaceType::kCuboid) {
    ...
    if (!pshim::param::get(param_name, radius)) {
    ...
    if (!pshim::param::get(param_name, radius_extension)) {
...
bool PlanningParams::loadParams(std::string ns) {
  ...
  if (!pshim::param::get(param_name, auto_global_planner_enable)) {
  ...
  if (!pshim::param::get(param_name, relaxed_corridor_multiplier)) {
  ...
  if (!pshim::param::get(param_name, interpolate_projection_distance)) {
  ...
  if (!pshim::param::get(param_name, go_home_if_fully_explored)) {

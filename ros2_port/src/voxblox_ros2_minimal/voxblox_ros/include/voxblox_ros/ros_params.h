...
inline TsdfIntegratorBase::Config getTsdfIntegratorConfigFromRosParam(
  ...
  // NTNU dev/noetic weighting params (sparsity_compensation removed upstream
  // in the fork GBPlanner actually pins; see ros2_port/docs/m2_scout_2).
  if (!node_ptr->has_parameter("clearing_ray_weight_factor")) {
    node_ptr->declare_parameter("clearing_ray_weight_factor",
  ...
  if (!node_ptr->has_parameter("use_symmetric_weight_dropoff")) {
    node_ptr->declare_parameter("use_symmetric_weight_dropoff",

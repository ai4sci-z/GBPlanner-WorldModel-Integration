...
**E1 可执行 CLI、registry、互斥恢复和证据门已实现;Codex 复现默认 --once 在 PENDING registry 上立即退出,且 run_id 仅在任务结束后 resolve,运行期连续旁路链未闭合。E1 实现停点 REOPENED,禁止 A/A**(Codex 判决 PARTIAL/VERIFIED_FAIL,2026-07-19)。
当前令 = R003-WP304-E1L(运行期接线纠偏;仍禁真实仿真/A/A/pilot)。
...
| G6 WP304 因果链 | 🔴 E1 实现停点 REOPENED | E1 可执行 CLI、registry、互斥恢复和证据门已实现;Codex 复现默认 --once 在 PENDING registry 上立即退出,且 run_id 仅在任务结束后 resolve,运行期连续旁路链未闭合。E1 实现停点 REOPENED,禁止 A/A(Codex PARTIAL/VERIFIED_FAIL);E1L 施工中 |
...
当前唯一施工点          下一停点                  解锁                    仍阻塞
──────────────────────────────────────────────────────────────────────────
E1L 运行期旁路链    →  E1 实现停点(再闭合)  →  A/A(另行放行)     →  E1 pilot/E2
(Codex 反例:--once 秒退                             ↳ OPEN-1 新数据
 /事后 resolve/事后快照/wait 阻塞/ROS 未实现)

...
| **WP304** | OPEN-1 因果时间线:样本分层 + F1-F5 + H1-H5 + E0/E1/E2;**E0 已收口**(open1/ 离线提取器 + CRC 校验协议解码器 + 观测 schema 草案 + 环境无关 fixture;证伪"accel=判别器";arm/no-BIN 死因=UNKNOWN;运行时埋点未实现) | WP303 | 逐阶段放行;E0 done,下一=进入 E1 最小旁路观测补丁方案停点(只写方案),运行时缺口捕获须先裁决扩权 | 🟡 **E0 收口停点:未启动仿真,申请进入 E1 方案停点** |

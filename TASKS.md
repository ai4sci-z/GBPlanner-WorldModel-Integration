...
| 9 | **⭐ B2.5 自写薄桥接真版 GBPlanner(当前主线)** | 🔵 **Stage2~4 全 PASS;当前=Stage5** | 官方桥/zenoh 判死→薄桥:transport✅、稳定性✅、消费闭环✅、dry-run✅、3D 数据链✅(voxblox 3D 体素 90,472点 zspan13.2m)、**Stage4 FCU 消费直证✅**(stage4b:/ap/v1/cmd_vel 出现 GBP-SIGNATURE,lin=-0.024/ang.z=-0.300 与适配器逐位吻合,唯一性已论证;fail-closed 安全门全套+kill 实测;XY/Yaw 低速)。**当前=Stage5**:策略替换(独占 intent 去混流)→gate/status 严格对齐→同口径对比(基线 40% 对照)+3D 行为对照;坐标系 NED/ENU 轴向映射实测校准。入口 docs/桥接查证与执行计划 |

...
| 3 | 预研A·完整复现并**实际运行** world-model | 🎉 **起飞突破(07-06)** | 干净复现(全新克隆作者09a5aa4)连修5类真bug,**无人机真物理离地**(SIM升0.75m/电机1950),**原版frontier_lite探索首次端到端**(飞1.61m/3目标/exploration_probe ok=True/takeoff.ok=True)。关键=死锁逻辑修复(B15)。就差 frame_contract_probe(/tf_static QoS)。clean分支 fix/world-model-e2e-takeoff |
...
| 12 | **⭐ jazzy 跑通 exploration(当前主线)** | 🎉 **起飞突破,frontier_lite 端到端跑起来(07-06)** | 干净复现 clean_repro.sh。连修5类真bug(%%/空launch/IMU回声/测距仪参数名/**死锁逻辑B15**),无人机真飞、frontier_lite探索probe通。**就差 frame_contract_probe 探针(/tf_static 需TRANSIENT_LOCAL QoS)**。⚠️曾堆参数hack被用户批已撤。接管文档 RESUME_新窗口接管_2026-07-06.md |

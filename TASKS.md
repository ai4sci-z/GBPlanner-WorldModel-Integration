...
| **WP304** | OPEN-1 因果时间线:样本分层 + F1-F5 + H1-H5 + E0/E1/E2;**E0 已收口**(open1/ 离线提取器 + CRC 校验协议解码器 + 观测 schema 草案 + 环境无关 fixture;证伪"accel=判别器";arm/no-BIN 死因=UNKNOWN;运行时埋点未实现;**E0-CORRECT 补正:逐输入 schema 门(UNSUPPORTED_SCHEMA)+ 标注冻结验收门(RAN=5/PASS=5/SKIP=0 + provenance 实绑)+ claim 语义时效门**) | WP303 | 逐阶段放行;E0+证据门/标注补正 done,E1 方案 done,**E1 可执行 CLI、registry、互斥恢复和证据门已实现;Codex 复现默认 --once 在 PENDING registry 上立即退出,且 run_id 仅在任务结束后 resolve,运行期连续旁路链未闭合。E1 实现停点 REOPENED,禁止 A/A**,当前=E1L 运行期接线纠偏 | 🔴 **E1 实现停点 REOPENED(Codex PARTIAL/VERIFIED_FAIL)** |

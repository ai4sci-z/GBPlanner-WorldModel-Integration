...
| **M2** | voxblox ROS2 后端落地 | 🔵 **切片 1-3 ✅(07-08 晚,feat 分支 d8b4ee1→3018307→aa66fcc)** | ①vendor snt-arg 底座@d08e9d4+Jazzy 全量构建 9/9 rc=0+esdf_server 冒烟 ✅ ②维护补丁(删幽灵依赖 rviz_plugin→7 包最小集;修 gflags 吃 --ros-args;**参数管道 E2E:param get 回读 map**)✅ ③**ntnu dev/noetic 行为等价补丁**(tsdf_integrator 全量移植+ros_params 三参数换血+sparsity×100 陷阱拆除;**gtest 10/10 PASSED**)✅。三路侦察报告 ros2_port/docs/m2_scout_*;证据 runbooks/ros2_port/m2_build*_evidence.txt。④**oracle 逐体素对拍双积分器全 PASS(切片4,1f61ff1)**:simple=46/46 块、35,323/35,323 观测体素零差、RMS 1e-4;fast=mismatch 2/34,506(竞态噪声);路上修掉真移植 bug(min_time 节流 1s 无操作)——**行为等价验收达成**(证据 runbooks/ros2_port/m2_oracle_cmp_evidence.txt)。**剩切片5=ESDF 对拍+world-model 场景建图+RViz2**(M2 收口)。**不用 nvblox** 原则不变 |

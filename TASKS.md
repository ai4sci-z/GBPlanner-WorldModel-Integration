...
| **WP304** | OPEN-1 因果时间线:样本分层 + F1-F5 + H1-H5 + E0/E1/E2;**E0 已收口**(open1/ 离线提取器 + CRC 校验协议解码器 + 观测 schema 草案 + 环境无关 fixture;证伪"accel=判别器";arm/no-BIN 死因=UNKNOWN;运行时埋点未实现;**E0-CORRECT 补正:逐输入 schema 门(UNSUPPORTED_SCHEMA)+ 标注冻结验收门(RAN=5/PASS=5/SKIP=0 + provenance 实绑)+ claim 语义时效门**) | WP303 | 逐阶段放行;E0+证据门/标注补正 done,E1 方案 done,**Codex 已复现:recover(telemetry_dir)=CORRUPT 但正式 evaluate_run_evidence() 仍使 evidence gate 得到 COMPLETE(段链漏验);且正式入口 sidecar 身份等待默认 5s 与 run_batch 120s 启动预算/300s watcher 不一致。E1L 状态 REOPENED/PARTIAL,A/A 不得放行。此为 E1 既有 required evidence 契约的失败,非冻结后新增范围** | 🔴 **E1L REOPENED/PARTIAL(Codex 复现段链漏验+预算不一致)** |
